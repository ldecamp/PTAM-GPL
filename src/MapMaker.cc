                              ...
                              SE3<> &se3TrackerPose)
{
  ...
  *pkSecond = kS;
  ...
  for (unsigned int i = 0; i < vMatches.size(); i++)
  {
    ...
    p->v3OneDownFromCenter_NC = unproject(mCamera.UnProject((first + Point2f(0, 1)).vec()));
    p->v3OneRightFromCenter_NC = unproject(mCamera.UnProject((first + Point2f(0, 1)).vec()));
                                ...
                                int nCandidate)
{
  ...
  for (unsigned int i = 0; i < vv2Corners.size(); i++) // over all corners in target img..
  {
    ...
    int dist = hamming.getScore(feat.descriptor, vIR[i].descriptor);
    if (dist < nBestDist)
    {
      nBest = i;
      nBestDist = dist;
  ...
  PatchFinder Finder;
  Finder.MakeTemplateCoarseNoWarp(kSrc, nLevel, feat.ptRootPos.ir());
  ...
  pNew->irCenter = feat.ptRootPos.ir();
